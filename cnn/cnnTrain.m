...
if DEBUG
    ...
    %disp([numGrad grad]);
    sizeWc = db_filterDim * db_filterDim * db_numFilters;
    sizebc = db_numFilters;
    sizebd = numClasses;
    outDim = imageDim - db_filterDim + 1;
    outDim = outDim / db_poolDim;
    hiddenSize = outDim ^ 2 * db_numFilters;
    sizeWd = numClasses * hiddenSize;
    startWd = sizeWc + 1;
    startbc = startWd + sizeWd;
    startbd = startbc + sizebc; 
    fprintf('\n\nWc\n')
    disp([numGrad(1 : sizeWc) grad(1 : sizeWc)])
    fprintf('\n\nWd\n')
    disp([numGrad(startWd : startWd + sizeWd - 1) grad(startWd : startWd + sizeWd - 1)])
    fprintf('\n\nbc\n')
    disp([numGrad(startbc : startbc + sizebc - 1) grad(startbc : startbc + sizebc - 1)])
    fprintf('\n\nbd\n')
    disp([numGrad(startbd : startbd + sizebd - 1) grad(startbd : startbd + sizebd - 1)])
    tempnumbd = numGrad(startbd : startbd + sizebd - 1);
    sum(tempnumbd(:))
    tempbd = grad(startbd : startbd + sizebd - 1);
    sum(tempbd(:))
